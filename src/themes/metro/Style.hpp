...
/// The standard widgets in the Windows 8 Modern UI: "content is chrome". Flat panels with light
/// section titles, cameras as live tiles in rotating accent colours, square 2px stroke controls
/// that fill with the accent when pressed, circular app-bar buttons, the selected tile marked with
/// an inner border and a check in its corner. In a dark or a light mode.
...
private:
    /// Inner stroke of @p width, kept inside @p rect (Metro strokes never blur across pixels).
    void stroke(QPainter& p, const QRect& rect, const QColor& color, int width) const;
